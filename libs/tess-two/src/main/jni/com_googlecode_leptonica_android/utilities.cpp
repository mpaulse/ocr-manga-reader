...
jboolean Java_com_googlecode_leptonica_android_Clip_nativePixTestClipToForeground(JNIEnv *env, jclass clazz,
                                                                                  jlong nativePix) {
  PIX *pixs = (PIX *) nativePix;
  int canClip = 0;
  int status = pixTestClipToForeground(pixs,&canClip);

  /* If error (1) or cannot clip */
  if(status == 1 || canClip == 0)
  {
    return JNI_FALSE;
  }
...
jlong Java_com_googlecode_leptonica_android_Clip_nativePixClipToForeground(JNIEnv *env, jclass clazz,
                                                                           jlong nativePix) {
  PIX *pixs = (PIX *) nativePix;
  PIX *pixd;

  if(pixClipToForeground(pixs,&pixd,NULL))
  {
    return (jlong) 0;
  }
...
/* cb4960 */
jlong Java_com_googlecode_leptonica_android_Seedfill_nativePixRemoveBorderConnComps(JNIEnv *env, jclass clazz,
                                                                                    jlong nativePix, jint connectivity) {

  PIX *pixs = (PIX *) nativePix;
  PIX *pixd;
  pixd = pixRemoveBorderConnComps(pixs,(int)connectivity);
  return (jlong) pixd;
}
...
/* cb4960 */
jlong Java_com_googlecode_leptonica_android_Convolve_nativePixBlockconvGray(JNIEnv *env, jclass clazz,
                                                                            jlong nativePix,
                                                                            jint wc, jint hc) {

  PIX *pixs = (PIX *) nativePix;
  PIX *pixd;
  pixd = pixBlockconvGray(pixs,0,(int)wc,(int)hc);
  return (jlong) pixd;
}
